...
현행 백업 schedule이 매일 밟는, 운영 검증된 경로다. Phase 2(ETF 08:15 전환)와 한 번에 설정한다.

| cron-job.org 작업 | 시각(KST, 월~금) | body |
|---|---|---|
| 마켓 정시(기존 수정) | 06:40 | `{"ref":"main","inputs":{"only":"market"}}` |
| 마켓 2차(신규) | 07:20 | 〃 |
| ETF 정시(신규) | 08:15 — 관측 확정(09-21~23 게시 08:00:41~47) | `{"ref":"main","inputs":{"only":"etf"}}` |
| ETF 2차(신규) | 08:40 | 〃 |
