# ETF 국내 시세 기준일 정확성 — KRX 기준일 가드 + 08:15 발송 전환

작성: 2026-09-20
상태: 설계 승인됨 (2026-09-20) · 관측 완료·발송 시각 08:15 확정 (2026-09-23) · Phase 2 구현 중
...
1. ETF만 08:15 KST 발송. 마켓은 06:40 유지.
...
> ETF 백업 cron(08:40 KST 의도)도 사실상 개장 후에 돌게 만든다 — Phase 2 착수 전에 백업 수단을
...
- 한국 휴장일이면 즉시 종료.
- 3~5거래일 로그(`gh run view --log`)로 게시 시각 분포를 보고 발송 시각을 확정한다. 관측이 끝나면 이 job은 제거한다.
  **관측 결과(2026-09-21~23)**: 게시 확인 08:00:41 / 08:00:43 / 08:00:47, 직전 미게시 07:55, 조회 실패 0회, 전량(1171~1175건).
  → 발송 시각 **08:15 KST**(여유 ~14분), 2차 트리거 08:40.
...
- cron-job.org(사용자 작업, 4개): 마켓 06:40·07:20 body `{"ref":"main","inputs":{"only":"market"}}`,
  ETF 08:15(관측 확정)·08:40 body `{"ref":"main","inputs":{"only":"etf"}}`. 모두 월~금, Asia/Seoul.
...
3. **가드 + 트리거 분리 + 문구**를 머지·push하고 **같은 날** cron-job.org를 전환한다.
   다음 날 아침부터 ETF는 08:15에만 돌고, KRX가 제때 게시되면 `prev-session` 경로(현행과 같은
