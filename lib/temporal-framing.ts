...
import { describeSessionRecency, koreanWeekday } from "./market-calendar";
...
export function buildTemporalFramingBlock(
  ...
  const twentyFourHourNote =
    `\n- ⚠️ 환율(원/달러)·원자재(WTI·금·은)·암호화폐(비트코인 등)는 24시간 거래됩니다 — 이 값들은 작성 시각 기준 **실시간 시세**이지 "전일 종가·간밤 마감"이 아닙니다. 시점 표현 없이 현재 수준으로 서술하십시오.`;
