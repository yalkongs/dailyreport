...
- 리포트는 하나의 중심 주제를 따라 작성하십시오. 중심 주제는 "해외 선행 신호가 국내 ETF 거래로 이어지는지 확인"처럼 명확해야 합니다.
- 모든 문단은 같은 논리 축을 이어받아야 합니다. "선행 신호 → 제약 조건 → 상품별 역할 → 확인 조건" 순서로 씁니다.
- ETF 상품은 단순 목록이 아니라 분석상 역할을 가져야 합니다. 예: 성장 신호 확인, 환율 영향 점검, 대안 관찰, 과열 경계.
- 리포트의 논리 순서는 "오늘의 중심 주제 → 결론 → 결론의 근거 → 상품별 역할 → 국내 ETF 실행 확인점 → 리스크"입니다.
...
- cover.headline은 단순 지수 요약이 아니라 오늘 리포트의 중심 주제로 씁니다. 45자 이내의 명사형 또는 간결한 설명문으로 작성합니다.
...
- 문장은 설명문으로 씁니다. "단계적 판단 대상입니다"처럼 확인 기준과 보류 조건을 함께 씁니다.
- "매수 후보", "일반 매수 후보", "접근 후보", "매수 검토", "신규 진입", "분할 접근", "비중 확대", "선호 ETF군"을 쓰지 마십시오. "관찰 대상", "점검 대상", "추가 편입 판단", "단계적 판단", "확인 우선 ETF군"으로 바꾸십시오.
- "전이"는 한 문단에 1회 이하로만 쓰고, 가능하면 "국내 거래로 이어지는지"로 바꾸십시오.
