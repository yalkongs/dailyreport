...
const e = escapeHtml
const IM_BANK_LOGO_PATH = path.join(process.cwd(), 'public', 'im-bank-signature-ko.jpg')
...
function imBankLogoSrc(): string {
  try {
    const image = fs.readFileSync(IM_BANK_LOGO_PATH)
    return `data:image/jpeg;base64,${image.toString('base64')}`
  } catch {
    return '../im-bank-signature-ko.jpg'
  }
}
