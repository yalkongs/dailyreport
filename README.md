...
- **클리셰 방지 = 규칙→취향** (2026-06): 단어 금지 리스트 대신 목표 보이스 예시+원칙(`lib/voice-exemplars.ts`)으로 결을 따르게 하고, anti-repetition이 단어뿐 아니라 구조·구문 틀("X가 그린 Y" 류) 반복까지 차단
...
### 아침 트리거 (cron-job.org → GitHub workflow_dispatch)
